...
function randomizeSlider(){
  PlantSlider.value = PlantSlider.value + Math.random()
}

function delaySliderChange() {
  setTimeout(randomizeSlider, 500);
}
delaySliderChange();

let updatePlant = () => PlantOutput.innerHTML = PlantSlider.value* 100;
PlantSlider.addEventListener('input', randomizeSlider);
updatePlant();
...
function predictWebcam() {
 ...
 //console.log(plantsDetected);
  ...
  /*      p.innerText = predictions[n].class  + ' - with ' 
    ...
    if (plantsDetected.length > 0 && predictions[n].score > PlantSlider.value) {
