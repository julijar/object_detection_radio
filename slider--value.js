...
const audioElementRadioStatic = document.getElementById('radioStatic');
const audioElementPlantMusic = document.getElementById('plantMusic');

// pass it into the audio context
const track = audioContext.createMediaElementSource(audioElementRadioStatic);
track.connect(audioContext.destination);

//loop the sounds
audioElementRadioStatic.loop = true; 
audioElementPlantMusic.loop = true;
...
function predictWebcam() {
 ...
 //console.log(plantsDetected);
  ...
  //    document.getElementById('PlantSliderDiv').style.color = "green";
    ...
    audioElementPlantMusic.play();

    /*  this.dataset.playing = 'true'; */
   
    } else if  (plantsDetected.length > 0 
      && plantsDetected[0].score-0.25<PlantSlider.value<plantsDetected[0].score-0.1
     && plantsDetected[0].score+0.1<PlantSlider.value<plantsDetected[0].score+0.25
      ) {
        audioElementPlantMusic.pause();
        audioElementRadioStatic.play();
   } else {
    audioElementRadioStatic.pause();  
    audioElementPlantMusic.pause();
